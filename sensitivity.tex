...
Whilst the vertical stopping distribution is largely symmetric (\fig{sense:stops:Y}),
...
This is almost exactly the value predicted by the analytic collimator approach of \sect{optim:BeamAndDIOBlocker}, suggesting that such an approach is truly adequate for this sort of study.
}.
...
Between production in the stopping target and detection at the \ac{StrECAL}, electrons lose energy by scattering, brehmstrahlung and ionisation of material in the electron's path, including the stopping target itself and the detector.
...
In that procedure, one would tune the cut to maximise the signal to background separation, as demonstrated by \fig{sense:integral} where the number of events accepted are shown as a function of the low-momentum cut value.
...
Although this function peaks at around 103.68~MeV/c, since this value is only optimal for the demonstration signal conversion of $R=\sci{3}{-16}$ and to bring the \ac{DIO} rate down further (see \sect{bg:dio}), a cut of \VarMomThreshold is selected.
