...
\CHECK{Add the muon momentum plot}
...
\CHECK{Is it worth adding the 1D projections of the stopping distribution?}
...
Since this value is only optimal for the demonstration signal conversion of $R=\sci{3}{-16}$ and to bring the \ac{DIO} rate down further (see \sect{bg:dio}), a cut of 103.9~MeV/c is selected.
...
Since our target \ac{ses} is fixed we instead tune the amount of running time needed to stop enough muons to achieve this goal, hence the by inversion of \eq{det:ses} we get the necessary run time as:
\begin{equation}
%\mathrm{S.E.S}(\muec)=\frac{1}{N_\mu \mathcal{B}_\mathrm{capture} A_{\mu\rightarrow e}}
	t_\textrm{run}=\frac{1}{(I_p/e){R}_{\mu/p}f_\mathrm{1s}\mathcal{B}_\mathrm{capture} A_{\mu\rightarrow e}\mathrm{S.E.S}},
\end{equation}
...
\Tab{sense:ses} lists the values of these parameters, from which we can see that a single signal event sensitivity of \sensePII can be achieved in around \num{1.57e7} seconds or about 180 days.
Alternatively, if we ran for some \VarRunTime[0] seconds, an \acf{ses} of \VarPredictedSES would be possible.
\Tab{sense:comparisons} compares this to the CDR estimate, which expected to achieve a \ac{ses} of \sci{2.6}{-16} in a run time of \sci{2}{7}~s, and \phaseI which is expected to reach its goal of \sensePI in about 140 days of running.
...
\begin{tabular}{lll}
	\hline
	 ...
	 ${R}_{\mu\textrm{-stop}}$      & \VarMuStopsPerPOT & Muon stopping rate per POT                     \\ 
...
\begin{table}[b]
\centering
        \begin{tabular}{m{0.25\textwidth}p{0.14\textwidth}p{0.13\textwidth}p{0.12\textwidth}p{0.23\textwidth}}
	\hline
                                                        & Single event sensitivity & Total \ac{POT} ($\times10^{19}$) &Beam time $t_\textrm{run}$ (s) & Sensitivity per year of continuous beam \\ 
  \hline
  COMET \phaseII\hspace{1ex}(this study)                & \VarPredictedSES         & 87 & \VarRunTime[2]                  & \VarPredictedSESPerYear                         \\ 
  COMET \phaseII\hspace{1ex}(CDR 2009~\cite{CDRphase2}) & \sci{2.60}{-17}           & 85 & \sci{2.00}{7}                  & \sci{1.65}{-17}                         \\ 
  Mu2e~\cite{Mu2e2014}                                  & \sci{2.40}{-17}           & 36 & \sci{6.00}{7}                  & \sci{4.57}{-17}                         \\ 
  COMET \phaseI~\cite{TDR2016}                          & \sci{3.00}{-15}           & 3.2 & \sci{1.26}{7}                  & \sci{1.19}{-15}                         \\ 
	\hline
\end{tabular}
        ...
        The sensitivity per year of continuous beam is the single-event-sensitivity that can be achieved in \sci{3.15}{7}~seconds of running, assuming no beam shutdown periods.
        }
\end{table}
%\newcommand\VarTotRunTime{2e7}
%\newcommand\VarTotPOT{8.8388e20}
%\newcommand\VarMuStopsPerPOT{1.61e-3}
%\newcommand\VarTotMuStops{\VarMuStopsPerPOT*\VarTotPOT}
%Total Muon stops=\sigfigs{\VarTotMuStops}
\CHECK{All of this needs updating with the probability of an atomically captured muon reaching the ground state.  Yoshi Kuno said it was about 90\%, but need to check}
