...
Since our target \ac{ses} is fixed we instead tune the amount of running time needed to stop sufficient muons to achieve this goal. Hence by the inversion of \eq{det:ses}, the necessary run time is fixed to:
\begin{equation}
...
\Tab{sense:ses} lists the values of these parameters, from which we can see that a single signal event sensitivity of \sensePII can be achieved in around \VarPredictedSES seconds or about 180 days.
