void tidy_plots(const char* format="png"){
  ...
  config.x_axis_label_size=config.y_axis_label_size = 0.04;
  config.x_axis_title_size=config.y_axis_title_size = 0.05;
  config.y_axis_label    = "Differential Yield ( #times 10^{-3} per MeV/c)";
  config.y_axis_decimal  = true;
  config.legend_text_size= 0.037;
  config.legend_x1  = 0.45 ; config.legend_y1 = 0.55;
...
//  return;
  ...
  // INTEGRATED SPECTRA
  config.reset();
  config.y_axis_label    = "Particle Yield per POT Below a Given Momentum (#kern[2.0]{ }#times 10^{-3}#kern[1.0]{)}";
  ...
  config.legend_text_size= 0.037;
  config.legend_x1  = 0.11; config.legend_y1  = 0.76;
  ...
  config.y_axis_title_size = 0.037;
  config.line_width=2;
