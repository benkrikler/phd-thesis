...
\centering
%\fbox{
\includegraphics[width=1.0\textwidth,trim=0 0 1cm 0.93cm,clip]{figs/backgrounds/Dio_BackgroundRate.pdf}
%}
\caption{\figlabel{bg:dio:rates}
The DIO background rate as a function of momentum threshold.
Each line show a different desired \ac{ses}.  
With that held fixed, the signal acceptance at a given threshold determines the number of muons that must be stoppped.
This, in turn, determines both the necessary run time (shown as the text labels along the line in units of \sci{1}{7} s) and the number of expected DIO background events.
\CHECK{This would be far nicer as lines of fixed running time, and SES on the text labels. Dammit...}
}
\end{figure}
}
