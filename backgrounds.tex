...
The data has also been converted to a differential rate of production, given the \phaseII production target dimensions, using equation~\eq{bg:antiproton:rate}.
...
Using the procedures described above, a separate simulation was performed for each of the four angular regions listed in \tab{bg:antiprotons:regions}, with \num{8e7} antiprotons fed in to each simulation.
Within each region, events were generated isotropically and according to the momentum distribution for that region.
The beamline's acceptance to antiprotons and the secondary pions that they produce is shown in \fig{bg:antiprotons:sim:fluxes}.
...
It is also clear, given that the transmission probability has a negative gradient for each angular region along the whole beamline, that the main source for pions is the interaction of antiprotons with the primary target.
Pions produced by antiprotons hitting collimators or the beampipe will be quickly removed themselves since they will tend to stay close to this material.
If pions were produced at the foil at the entrance to the Torus1 then they would be in the centre of the beampipe and have a greater chance of being transported.
However, since the pion rate at that point does not seem to increase, based on this work, it does not appear that this is a significant source of pions.
...
One can see then, that although the acceptance per antiproton is considerably lower for small angle production (0--59\degree), the production rate is large enough to offset this.
As such, it is the antiprotons produced in the forwards direction, followed by a hard scatter within the production target, that will dominate the background rate.
...
Also shown is the sum over all events with each event weighted by the initial antiproton direction, and normalised to the number of antiprotons introduced into the simulation.
...
The total predicted antiproton rate per \ac{POT} is therefore \VarAntiprotonsPerPOT, which is completely dominated by antiprotons produced in the forward direction but scatter within the production target and are successfully transported to the stopping target.
...
The rate of observing signal-like electrons due to high-energy beam particles is modelled by factorisation of the acceptances:
\begin{align}
...
Finally, then, we can pull all these factors together using \eq{bg:beam:factors}, from which we find that the predicted beam-related background rate per POT is~\VarBeamBgPerPOT, such that we can expect about \VarBeamBgTotal background events during \phaseII due to these processes.

Given the extrapolation that has been necessary, this result comes with a degree of statistical uncertainty.
...
In addition these neutrons should be studied carefully using multiple hadron production models.
\CHECKmargin{Add at least a comment or subsection  on these neutrons and reference it here.}
...
\phaseII is expected to run about \VarRunTime~s as suggested in section~\sect{sensitivity}.
...
\Fig{bg:cosmic:primary} shows the distributions of momentum and directions that these files contained.
...
\Fig{bg:cosmic:momenta} shows the momentum distribution of observed electrons.  
...
\Tab{bg:summary} gives an overview of the predict rates.
The final prediction is below 1 but very close to it, and it is therefore important that these studies be continued and developed further.
The dominant backgrounds come from antiprotons and cosmic rays, with radiative muon capture similarly high.
...
The following list, therefore, is a summary of methods that can be pursued to improve and expand the preceding set of estimates.
