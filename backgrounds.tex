...
Given the \VarTotalMuStops~muon stops that should take place during \phaseII, the expected number of background events due to DIO is therefore \VarDIOTotal.
...
The two most recent calculations of the high energy tail of the DIO spectrum are shown in \fig{bg:dio:spectra}, as well as the cruder function that fits the whole spectrum range that is contained in SimG4.
The more conservative spectrum from the 2011 paper has been used in this study.
However, if one were to use the more recent 2015 spectrum, background rates due to DIO would fall to 0.19 at the same momentum threshold.
