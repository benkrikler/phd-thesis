...
In 2013, when I first worked on the COMET experiment, many disparate and stand-alone simulations were being used without a common approach in the data structures and analysis.
...
In the 3 years since the summer of 2013 and the initial work to fork ND280 to ICEDUST, some 3,200 commits have edited about two million lines of code in the official version of the framework.
...
Simulated data is also produced in the oaEvent format and involves some 4 to 6 packages being called, described in more depth in the next section.

Once either simulated data has been produced or real data has been converted, calibration routines can then be applied.
Each sub-detector's routine is capable of pulling previously-generated constants from an SQL database and applying these to the detected (or simulated) energy deposits.
...
The dipole field is shown in \fig{software:field:dipole}.
...
Default Geant4 uses nuclear capture rates based on a 1987 paper by Suzuki \etal~\cite{Suzuki1987}, filling in experimental blanks with a semi-empirical model, and decay rates based on a largely theoretical 1977 model by Mukhopadhyay~\cite{Mukhopadhyay:1976hu}.
\Fig{detector:mu-nucl-params} was produced by extracting these parameters.

\FigSimulationPhysicsClasses
To extend the default Geant4 modelling and include the improved theoretical understanding of the \ac{DIO} process and measurements of muon nuclear capture, three new classes have been written which mirror the way the default Geant4 model is implemented but add more detail.
...
For each possible secondary, both a spectrum and the total production probability per capture must be supplied.
...
\section{Analysis Using the Beamline Coordinate System}
...
\Fig{software:beamline} shows the beamline and horizontal coordinate for points in the X-Z plane changes for the \phaseI geometry.
...
This technique is used extensively later in this thesis, to help study and optimise \phaseII.

\FigSoftwareBeamlineAnalysis

\Fig{software:beamline:analysis} shows two projections to the beamline coordinate system of simulated data.
In \fig{software:beamline:analysis:height}, the density of simulated trajectories is shown as a heat-map and as a function of both distance along and the height relative to the beamline is shown.
Overlaid on this plot is a slice through the geometry to help interpret where in the experiment each effect occurs.
Each trajectory in the simulation is persisted as a list of points along the particle's path.
To make such a plot, therefore, one has to force Geant4 to produce a higher-than-normal density of points along each trajectory.
However, a step point is always inserted when a particle crosses a boundary in the geometry of the simulation, creating a spike in the density of points at a single surface in the plot, which does not correspond to a physical increase in the particle flux.
...
\Fig{software:beamline:analysis:flux} shows a one-dimensional flux plot, \ie the number of particles passing a given point along the beamline.
...
%\begin{easylist}
%	# Beamline coordinate
%	## The global COMET coordinate system
%	## The bent form of COMET, and the fact that key components are orientated differently to one another means a beamline coordinate system is useful
%	### Defined as the distance along the projection to the beamline axis
%	## Implemented into ICEDUST using a single class to facilitate transformations between COMET global and beamline coordinates, as well as perform operations such as obtaining the direction vector for the beamline at a given point
%	## The function to go from beamline to global and vice versa extract the important geometric parameters from the geometry files so that if the geometry changes the beamline coordinate system will still function
%	## Allows for many novel approaches to analysis, especially for truth level data, in ways not previously possible
%	# Flux plots and projected plots
%	## Heat map showing density of trajectories projected onto the Y-beamline axis surface
%	## However, in the simulation a trajectory is persisted as a series of step points. The space between step points can be limited, but additional points from geometric boundaries force steps to be placed, an creating an artificially high density of points along a single surface.  This creates artefacts in the plot which would be very difficult to remove.  They don't impact the intepretation too much, but do complicate things
%	## A flux plot is a form of 1-dimensional projection, showing the number (or fraction) of particles that pass a given point along the beamline. 
%	## Due to the above effect however, simply histrogramming the number of step points at each beamline distance would have very large spikes.
%	## Instead, the start and end point of each trajectory is identified on the beamline axis, and then every bin in between these points filled with the same weight.
%	## This works well for trajectories that move at constant longitudinal velocity along the beamline, but introduces an issue when trajectories slow down (which should increase the density of step points) and especially if the particle is mirrored, in which case points will only appear on the same side of the start point as the finish point, and no bins will be filled in the region where the mirroring occurred.
%\end{easylist}
