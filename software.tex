...
\Fig{software:geom:screenshots} shows some images of the geometry that has been created using this scheme.
...
Although the AlCap result is still only preliminary, the spectrum observed there is much softer than produced by standard Geant4 as shown in \fig{software:customPhysic:ProtMuCap}.
...
Control passes therefore over to the `COMETMuonMinusCapture' class, which contains an instance of the default Geant4 Bertini Cascade model to use for materials other than aluminium, although even this can be re-enabled at run-time.
