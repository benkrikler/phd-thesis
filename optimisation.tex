...
That study found that a \ac{ses} of $2.6\times10^{-17}$ could be achieved in $2\times10^{7}$ seconds of running, with a total expected background count of fewer than 0.34 events over the entire run period.
...
The solenoidal field and solenoid aperture could also be optimised in principle, but this is considered beyond the scope of the present study, and so we use here the same values for the aperture and solenoidal field strength as for the CDR.
...
Given the complexity of the optimisation in this region, it was decided to keep the stopping target at the nominal location for the subsequent steps.
