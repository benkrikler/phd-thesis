...
The outputs of this optimisation then should not be considered as final but should instead be treated as a baseline from which more intelligent approaches or physicists can improve.
...
The drift of a particle due to the dipole field is proportional only to the time spent in that dipole field and the dipole field strength, and does not depend on the particle's momentum.
This means that the total drift due to the Torus1 and Torus2 dipole components is proportional to the weighted sum of the two dipole field strengths, where the weight is the time spent passing through each of the two sections.
...
This leads to the total drift being proportional to the sum of the two dipole fields, and since the production target (as the source of muons) and stopping target (as the muons' destination) are at a fixed heights with respect to one another, the stopping rate is proportional to the total drift.

With that said, the Torus1 section has a higher pion flux which causes some assymetry between the two sections.
Keeping pions on-axis in the Torus1 section means that more muons enter the Torus2 section.
This could explain the slight assymetry seen and the fact that the apparent gradient of the correlation is not quite unity (which would imply equal time spent in each of the two sections if the beam were purely muons).
